...
X = data_scaled[:, :-1]
y = data_scaled[:, -1]
learningrate = np.array([x * 0.01 for x in range(5, 25, 2)])

learningrate = list(np.around(learningrate, 3))
print(learningrate)


nestimator = list(np.arange(50, 100, 10))
...
xc = results["param_learning_rate"].to_numpy()
ye = results["param_n_estimators"].to_numpy()
vm = abs(results["mean_test_score"].to_numpy())
print(xc)
results = pd.DataFrame(
    results, columns=["param_learning_rate", "param_n_estimators", "mean_test_score"]
)
results.to_csv("gbroptimize\GBR遍历lr_nestimator.csv")
