...
results = pd.read_csv("ANNoptimize\ANN遍历.csv")
print(results)
results = results.pivot(
    index="learning rate", columns="hidden node number", values="MAPE in test"
)
plot = sns.heatmap(results, cmap="viridis").invert_yaxis()

plt.xlabel("hidden node number in two layers")

plt.ylabel("learning rate")
plt.savefig("ANNoptimize\ANN2遍历lrnodenum.svg")
