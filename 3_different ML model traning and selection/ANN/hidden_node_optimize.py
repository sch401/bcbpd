...
firstlayerinput = int(sys.argv[1])
...
def traink(
    ...
    net = Activation_Net(X_train.size(1), firstlayer, secondlayer, 1)
...
def k_fold(k, X, y, learning_rate, firstlayer, secondlayer):
    ...
    for i in range(k):
        ...
        # print(data[1])
        loss444, pred1 = traink(*data, 5000, learning_rate, firstlayer, secondlayer, i)
        loss444.columns = [
            "mse in train",
            "mse in test",
            "mape in train",
    ...
    plt.gcf().text(
        ...
        0.8,
        "R$^2$ = %.4f"
        % (r2_score(pred["actual normalized EMY"], pred["predicted normalized EMY"])),
        fontsize=18,
    )
    ...
    sns.barplot(loss_combined, x=np.arange(1, 11, 1), y=loss_combined["MAPE in test"])
...
k_fold(10, X, y, 0.01, firstlayerinput, secondhyddenlayerinput)
